...
const rent = () => {
  ...
  // 月租模式
  if (model.value === 1) {
    if (!periods1.value) return alert("请选择首交期数");
    const coefficientsFind = coefficients.find(
      (el) => el.key === periods.value && el.key1 === periods1.value
    )?.value;
    const rent = new Calculator().divide(price.value, coefficientsFind).getResult();
